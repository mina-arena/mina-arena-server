...
export default async (parent, args, contextValue, info) => {
    return await sequelizeConnection.transaction(async (t) => {
        ...
        if (!game)
            throw new Error(`No Game found with ID ${args.input.gameId}`);
...
async function validateGame(game, t) {
    // Validate that game is in correct state to begin
    if (game.status != 'pending')
        throw new Error(`Game ${game.id} cannot be started, is in status ${game.status}`);
    let gamePlayers = await Models.GamePlayer.findAll({
        where: { gameId: game.id },
        transaction: t
    });
    if (gamePlayers.length < MIN_PLAYERS || gamePlayers.length > MAX_PLAYERS) {
        throw new Error(`Invalid number of GamePlayers to start Game (${gamePlayers.length}), ` +
    ...
    for (const gamePlayer of gamePlayers) {
        ...
        if (pieces.length < 1 || pieces.length > MAX_PIECES) {
            throw new Error(`Invalid number of GamePieces selected for player ${gamePlayer.player.name} ` +
...
async function setupGame(game, gamePlayers, t) {
