...
export default async (parent, args, contextValue, info) => {
    return await sequelizeConnection.transaction(async (t) => {
        ...
        if (!player)
            throw new Error(`Player not found with provided public key`);
        ...
        if (gamePhase.id != currentPhase.id)
            throw new Error(`GamePhase ${gamePhase.id} cannot be submitted because it is not the current phase for Game ${game.id}`);
        // Resolve the GamePhase and return the Game
        return await gamePhase.resolve(t);
    });
};
//# sourceMappingURL=submit_game_phase.js.map