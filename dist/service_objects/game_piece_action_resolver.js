...
export default async (action, commitChanges = false, parentTransaction) => {
...
export async function resolveMoveAction(gamePiece, moveFrom, moveTo, commitChanges = false, transaction) {
...
export async function resolveRangedAttackAction(attackingGamePiece, targetGamePieceId, commitChanges = false, transaction) {
...
export async function resolveMeleeAttackAction(attackingGamePiece, targetGamePieceId, commitChanges = false, transaction) {
    // Confirm target GamePiece exists and is a valid target
    const targetGamePiece = await Models.GamePiece.findByPk(targetGamePieceId, { transaction: transaction });
    if (!targetGamePiece)
        throw new Error(`No GamePiece found for targetGamePieceId ${targetGamePieceId}`);
    ...
    if (distanceToTarget > attackerRange)
        throw new Error(`GamePiece ${attackingGamePiece.id} cannot execute a melee attack against target GamePiece ${targetGamePiece.id} because distance ${distanceToTarget} is greater than attacker's max range of ${attackerRange}`);
    if (commitChanges) {
        // Validations done, modify state
        if (targetGamePiece.isDead())
            return attackingGamePiece;
